...
contract RaffleIntegrationTest is Test {
    ...
    function testRevertPerformUpkeepWhenConditionsNotMet() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(Raffle.Raffle__UpkeepNotNeeded.selector, 0, 0, uint256(Raffle.RaffleState.OPEN))
        );
        raffle.performUpkeep(bytes(""));
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Raffle.Raffle__UpkeepNotNeeded.selector, config.entranceFee, 1, uint256(Raffle.RaffleState.OPEN)
            )
        );
        raffle.performUpkeep(bytes(""));
    }
    ...
    function testRevertPerformUpkeepWhenAlreadyCalculating() public skipFork {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Raffle.Raffle__UpkeepNotNeeded.selector, config.entranceFee, 1, uint256(Raffle.RaffleState.CALCULATING)
            )
        );
        raffle.performUpkeep(bytes(""));
    }
